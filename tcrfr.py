...
class AbstractTCRFR(object):
    ...
    v = None  # parameter vector of the crf (consisting of transition part and emission part)
    ...
    def __init__(self, data, labels, label_inds, unlabeled_inds, states, A, reg_theta=0.5, reg_lambda=0.001, reg_gamma=1.0):
        ...
        print('Creating TCRFR with #{0} examples and #{1} features.'.format(self.samples, self.feats))
        print('Dims:')
    ...
    def log_partition_derivative(self):
        pass



class TCRFR(AbstractTCRFR):
    ...
    def __init__(self, data, labels, label_inds, unlabeled_inds, states, A, reg_theta=0.5, reg_lambda=0.001, reg_gamma=1.0):
        AbstractTCRFR.__init__(self, data, labels, label_inds, unlabeled_inds, states, A, reg_theta=0.5, reg_lambda=0.001, reg_gamma=1.0)
    ...
    def unpack_v(self):
        states = self.S
        upv = np.zeros(states*states + self.S*self.get_num_feats())
        trans = np.zeros((states, states))
        cnt = 0
        for s1 in range(states):
            for s2 in range(s1, states):
                trans[s1, s2] = self.v[cnt]
                trans[s2, s1] = self.v[cnt]
                cnt += 1
        upv[:states*states] = trans.reshape(states*states)
        upv[states*states:] = self.v[cnt:]
        return upv
    ...
    def log_partition(self):
        ...
        v_trans = self.v[:self.S*self.S].reshape((self.S, self.S), order='F')
        v_em = self.v[self.S*self.S:].reshape((self.feats, self.S), order='F')
