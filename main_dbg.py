...
def get_test_data(exms, train):
    ...
    qp   = TCRFR_QP(x.T.copy(), y[linds].copy(), linds, uinds, states=2, A=A, reg_theta=0.5, trans_sym=[1])
    bf   = TCRFR_BF(x.T.copy(), y[linds].copy(), linds, uinds, states=2, A=A, reg_theta=0.5, trans_sym=[1])
    return lbpa, qp, bf, x, y, z


def test_bf():
    lbpa, qp, bf, x, y, z = get_test_data(10, 6)
